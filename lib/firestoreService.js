 ...
 * Se llama solo al finalizar un partido.
 */
const actualizarEstadisticasEquipo = async (equipoId, gano, gamesFavor, gamesContra) => {
    ...
    // Usamos query/where para obtener el documento con su ID (alternativa a getDoc)
...
export const getPartidosPendientes = async () => {
  try {
    ...
    const partidosList = snapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
      // Convierte el Timestamp de Firebase a una cadena para compatibilidad con HTML/JS
      horaInicio: doc.data().horaInicio ? doc.data().horaInicio.toDate().toISOString().substring(0, 16) : null
    }));
...
export const finalizarPartidoYActualizarRanking = async (partido, resultadoSets) => {
    ...
    // 1. Calcular sets y games totales
    resultadoSets.forEach(([scoreA, scoreB]) => {
        if (scoreA > scoreB) {
            setsA++;
    ...
    await actualizarEstadisticasEquipo(partido.idEquipoB, ganoB, gamesB, gamesA);
